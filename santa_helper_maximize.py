        ...
        while len(self.toys) > 0:
            ...
            if elf.rating > target_rating:
                ...
                if toy_duration < (time_left * 1.57) and self.toys[toy_idx + 1].duration > (600 * target_rating):
                    ...
                    trial_toy_idx = self.find_best_fit(elf.rating, time_left, toy_idx)
                    if (trial_toy_idx > 0) and time_left >= 600:
                        #if elf.id == 1:
                            #logging.info('---{0}'.format(trial_toy_idx))
                        if elf.id == 1:
                            logging.info("XXX")
                        toy_idx = trial_toy_idx
                    else:
